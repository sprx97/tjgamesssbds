class Mario: public Fighter {
	...
	public:
	// constructor
		Mario(int xpos, int ypos, int num, vector<Fighter*> listplayers, Display *disp, bool AI = false) {
			display=disp;
			players=listplayers;
			shieldstr = 64;
			myledge = -1;
	...
	// actions
		...
		void bdown() {
			if(action != BDOWN) {
				...
				projectiles.push_back(Projectile(x, y-16, -5*directionmodifier, 0, 2*fluddcharge, FLUDDWATER, charnum, tempbox, stage, display));
				fluddcharge = 0;
			}
