class Kirby: public Fighter {
	int rockcount;
	public:	// constructor
		Kirby(int xpos, int ypos, int num, vector<Fighter*> listplayers, Display *disp, bool AI = false) {
			display=disp;
			players=listplayers;
			shieldstr = 64;
			myledge = -1;
			...
			if(action != BUP) {
				...
				if(delay == 1 && PA_GetSpriteAnimFrame(MAIN_SCREEN, SPRITENUM) == 199) {
					...
					if(!checkFloorCollision()) {
						...
						projectiles.push_back(Projectile(x, y, -5*directionmodifier, 0, 20, FINALCUTTER, charnum, tempbox, stage, display));
					}
				}
			}
		}
